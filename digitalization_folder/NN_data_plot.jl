...
function AveragePerStepACC(data,bs,step)
    acc_up, acc_down, vld_up, vld_down, time = dataloader(data)

    
    avg_acc_up   = [100*(1-sqrt(c_avg(acc_up  ,sp = i-step+1, ep = i )))  for i in range(step,length(acc_up  ),step = step)]
    avg_vld_up   = [100*(1-sqrt(c_avg(vld_up  ,sp = i-step+1, ep = i )))  for i in range(step,length(vld_up  ),step = step)]
    avg_acc_down = [100*(1-sqrt(c_avg(acc_down,sp = i-step+1, ep = i )))  for i in range(step,length(acc_down),step = step)]
    avg_vld_down = [100*(1-sqrt(c_avg(vld_down,sp = i-step+1, ep = i )))  for i in range(step,length(vld_down),step = step)]
    x = [i*step for i in 1:length(avg_acc_up)]

    plot( x, avg_acc_up,  label="averaged training accuracy top face"       ,c=col[1])
    plot!(x, avg_vld_up,  label="averaged verification accuracy top face"   ,c=col[4])
    plot!(x, avg_acc_down,label="averaged training accuracy bottom face"    ,c=col[2])
    plot!(x, avg_vld_down,label="averaged verification accuracy bottom face",c=col[3])
    plot!(  yscale=:log10,
            minorgrid =true,
            minortick=true,
            xlabel= "Epochs",
            ylabel= "Accuracy [%]", yguidefontrotation = -90,
            title="Training and verification loss averaged per $step Epochs \n Batch Size = $bs ",
            legend= :topright,size=(1000,9/16*1000))
    ylims!((0.9*minimum((minimum(acc_up),minimum(acc_down),minimum(vld_up),minimum(vld_down))),1e-2))
    display(plot!())  
...
AveragePerStep(data_025,025,st)
a = AveragePerStep(data_050,050,st)
b = AveragePerStep(data_100,100,st)
c = AveragePerStep(data_150,150,st)
d = AveragePerStep(data_200,200,st)

a_acc = AveragePerStepACC(data_050,050,st)

x = [i*st for i in 1:length(a[1])]
labels=["training top side","validation top side","training bottom side","validation bottom side"]
#------ Average Error --------
for i in [1,3]
plot(title = "Comparison between the different Batch Sizes",yaxis = :log10, 
    xlabel= "Epochs", 
    ylabel= "Mean Square Error", yguidefontrotation = 0,)
...
#----- Average Accuracy -------
plot(title = "Averaged Accuracy History", 
    xlabel= "Epochs", 
    ylabel= "Accuracy [%]", yguidefontrotation = 0,minorgrid = true,minorticks = true)
plot!(x,a_acc[1], c = col[1], label = labels[1])
plot!(x,a_acc[2], c = col[4], label = labels[2])
plot!(x,a_acc[3], c = col[3], label = labels[3])
plot!(x,a_acc[4], c = col[2], label = labels[4])
savefig(path*"images/Conv_ACC.pdf")

plot(title = "Comparison between the different Batch Sizes",xlabel= "Epochs", 
    ylabel= "Time [sec]", yguidefontrotation = 0,)
